# 3_plot_trend_year.R
# plot the trend over time in text probability 
...
here = getwd()
setwd('\\\\hpc-fs\\barnetta\\acronym\\data') # move to lyra
rds.files = dir(pattern='.rds')
not.rds.files = dir(pattern='.excludenum') # the files exclude the top 100
...
setwd(here) # move back

## arrange into panels
to.plot = dplyr::filter(all.trends, 
                 year >= year.start,
#                 exclude == 'None',
...
print(wplot)
dev.off()
# export for Zoe (oct 2020)
to.export = filter(to.plot, 
                   n.chars=='allchars',
                   journals=='alljournals',
                   only.letters==FALSE) %>%
  select(-journals, -only.letters, -facet, -n.chars, -exclude)
write.csv(to.export, file='data/acronym_trend.csv', quote=FALSE, row.names = FALSE)
